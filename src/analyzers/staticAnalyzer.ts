...
import {
  ...
  verificarUseStateExcesivo,
  verificarImportsMuertos,
  verificarAnyType,
  verificarNonNullAssertion,
} from './static/staticCodeRules';
...
export function analizarEstatico(
  ...
  for (const regla of reglas) {
    if (!reglaHabilitada(regla.id)) { continue; }
    if (!regla.aplicaA.some(ext => ext === extension || ext === 'todos')) { continue; }

    /* Excluir barras-decorativas en Glory/ */
    if (regla.id === 'barras-decorativas') {
      const rutaNorm = documento.fileName.replace(/\\/g, '/');
      if (rutaNorm.includes('/Glory/')) { continue; }
    }
...
function ejecutarReglaPorLinea(
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    if (i > 0 && lineas[i - 1].includes(`sentinel-disable-next-line ${regla.id}`)) { continue; }
    if (linea.includes(`sentinel-disable ${regla.id}`)) { continue; }
    ...
    if (match) {
      ...
      if (match[1]) { mensaje = mensaje.replace('$1', match[1]); }
...
function ejecutarReglaCompleta(
  ...
  while ((match = patron.exec(texto)) !== null) {
    ...
    if (posicion.line > 0) {
      ...
      if (lineaAnterior.includes(`sentinel-disable-next-line ${regla.id}`)) { continue; }
    }
